...
BEGIN
	DECLARE X INT;
	SET X = (SELECT COUNT(RUT) WHERE RUT = RUT AND USERNAME = LOWER(USERNAME));
	IF(X = 0) THEN
		INSERT INTO USERS values (RUT,NAME,LASTNAME,USERNAME,MAIL,1,1,CAREER,PHONE,ADDREES,PASS);
		RETURN 1;
	ELSE
		RETURN 2;
	END IF;
...
DELIMITER //
CREATE OR REPLACE FUNCTION FN_DISABLE_USER
(
	RUT		INT
)
RETURNS INT 
BEGIN
	DECLARE X INT;
	SET X = (SELECT COUNT(RUT) FROM USERS WHERE RUT=RUT);
	IF(X=1) THEN
	
		UPDATE USERS SET STATUS = 0 WHERE RUT= RUT;
		RETURN 1;
	ELSE
		RETURN 2;
	END IF;
...
BEGIN
	DECLARE X INT;
	SET X = (SELECT COUNT(RUT) FROM USERS WHERE USERNAME = LOWER(USERNAME) AND PASS = PASS);
	IF (X = 1) THEN
		RETURN 1;
	ELSE 
		RETURN 2;
...
RETURNS INT
BEGIN
    INSERT INTO PRODUCT VALUES(NAME,DESCRIPTION,1,STOCK,PRICE,CELLAR);
    RETURN 1;
end
//
...
BEGIN
	DECLARE X INT;
	SET X = (SELECT COUNT(ID) FROM PRODUCT WHERE ID=ID);
	IF(X=1) THEN
...
RETURNS INT
BEGIN
    INSERT INTO MOVEMENT_HEADER VALUES(DATE_BEGIN,DATE_END,DAYS,USER_M,DESCRIPTION,0,0);
    RETURN 1;
END
//
...
RETURNS INT
BEGIN
    INSERT INTO MOVEMENT_BODY VALUES(PRODUCT_M,HEADER);
    RETURN 1;
END
//
...
RETURNS INT
BEGIN
	UPDATE MOVEMENT_HEADER SET STATUS = STATUS WHERE ID = ID;
	RETURN 1;
END
//
